...
class ModeratorCog(commands.Cog, name = "Moderator"):
    ...
    async def ban(self, ctx, member: discord.Member, *, reason = None):
        ...
        if member.id == ctx.author.id: return await ctx.send("You can't do that to yourself!")
        if member.top_role >= ctx.me.top_role: return await ctx.send("That member's top role is higher or equal to mine!")
    ...
    async def block(self, ctx, user: discord.Member):
        ...
        if user == ctx.author.id: return await ctx.send("You can't do that to yourself!")
        if user.top_role >= ctx.me.top_role: return await ctx.send("That member's top role is higher or equal to mine!")
        if user.top_role >= ctx.me.top_role: return await ctx.send("That member's top role is higher or equal to yours!")
        channel = ctx.channel
    ...
    async def saveban(self, ctx, member: discord.Member, *, reason = None):
        ...
        try:
            await member.send(
            embed = discord.Embed(description=f"You were banned from {guild.name} for {audit}")
            )
        except discord.HTTPException:
            pass
