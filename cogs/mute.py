...
class MuteCog(commands.Cog):
    ...
    async def mute(self, ctx, member:discord.Member, *, time:TimeConverter = None):
        ...
        if not role: return await ctx.send("The muted role doesn't exist, run `mute_setup` to make another.")
        if role > ctx.me.top_role: return await ctx.send("The muted role is above mine!")
        if role in member.roles: return await ctx.send("This user already has the muted role!")
        if role > ctx.author.top_role: return await ctx.send("Your role is not above the Muted role!")
    ...
    async def unmute(self, ctx, member:discord.Member):
        role = discord.utils.get(ctx.guild.roles, name="Muted")
        if not role: return await ctx.send("The muted role doesn't exist, run `mute_setup` to make another.")
        if role not in member.roles: return await ctx.send("This user doesn't have the muted role!")
        if role > ctx.me.top_role: return await ctx.send("The muted role is above mine!")
        if role > ctx.author.top_role: return await ctx.send("Your role is not above the Muted role!")
        if member.top_role >= ctx.author.top_role: return await ctx.send("The member's top role is above yours!")
        await member.remove_roles(role)
    ...
    async def mute_setup(self, ctx):
        ...
        if mutedRole > ctx.me.top_role: return await ctx.send("The muted role is above mine!")
